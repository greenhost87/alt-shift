...
import { GenerationError } from '../../src/system/generation/errors';
...
test('ignores additional fields in upstream delta events', async () => {
  const encoder = new TextEncoder();
  respondWithChunks([
    encoder.encode(generationFixtures.tolerantFirstChunk),
    encoder.encode(generationFixtures.tolerantSecondChunk),
  ]);
  ...
  expect(letter).toBe(generationFixtures.tolerantLetter);
});
