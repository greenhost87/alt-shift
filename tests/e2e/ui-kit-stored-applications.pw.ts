...
test('stored application scrolls its letter except on phones', async ({ page }) => {
  ...
  for (const width of [600, 768, 1024]) {
    ...
    await expect(letter).toHaveCSS('scrollbar-gutter', 'stable');
    expect(await letter.evaluate((element) => element.scrollHeight > element.clientHeight)).toBe(
      true,
    );
    await expect(panel).toHaveAttribute('aria-label', 'Saved letter, scrollable');
    await letter.hover();
    await page.mouse.wheel(0, 500);
    await expect
      .poll(async () => letter.evaluate((element) => element.scrollTop), { timeout: 5_000 })
      .toBeGreaterThan(0);
