...
test('switches to Russian, persists the locale, and submits it for generation', async ({
  ...
  const homeButton = page.getByRole('link', { name: 'My letters' });
  ...
  const languageCenter = (languageSelectBox?.y ?? 0) + (languageSelectBox?.height ?? 0) / 2;
  const homeCenter = (homeButtonBox?.y ?? 0) + (homeButtonBox?.height ?? 0) / 2;
  expect(Math.abs(languageCenter - homeCenter)).toBeLessThanOrEqual(2);
