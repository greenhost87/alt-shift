...
function getHeaderValue(headers: Headers, name: string): string {
  const maxLength =
    getPositiveIntegerEnv('CLIENT_FINGERPRINT_HEADER_MAX_LENGTH') ?? DEFAULT_MAX_HEADER_LENGTH;
  return (headers.get(name) ?? '').trim().slice(0, maxLength);
}
